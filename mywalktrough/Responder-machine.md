...
The Plan (in our heads):
    ...
    "Okay, Nmap's running. Probably some web server, maybe SSH, let's see what we're working with."
...
The Immediate Thoughts:
    ...
    Port 5985 → WinRM. That's literally a backdoor if we get creds.
...
The Fix: Add 10.129.112.205 unika.htb to /etc/hosts and pray.
...
The Aha Moment: We notice the language switcher on the page. Clicking a flag changes the URL to ?page=.... We try ?page=../../../../../../windows/win.ini and BOOM — we get the Windows win.ini file. LFI confirmed.

Facepalm Moment #3: We try to read C:\Users\Administrator\Desktop\flag.txt directly via LFI, but it's not there. Wait... we're not even on the right desktop. There's a mike user. The flag is on mike's desktop? Sus.

But now we have LFI, and Windows is on the other side. And you know what that means? UNC Path Injection + Responder = Free Hashes. This is where the fun begins.
...
So we do this:
    ...
    Start Responder on our attacker machine:
...
We got the Administrator's hash.
Cracking the Hash
...
Password: badminton. Of course it's badminton. Classic.
Shell Time (WinRM)
...
So you're a sysadmin and you just watched a hacker walk into your Windows server with a password from a wordlist. Here's how to stop this from happening:
1. Fix the LFI Vulnerability

    Root Cause: The web app accepts user-controlled filenames in the page parameter without sanitization.
    ...
    Better: Don't include files dynamically at all. Use a proper templating system.

2. Enforce Strong Passwords

    Root Cause: badminton is in rockyou.txt. That's tragic.
...
3. Secure WinRM
    ...
    Root Cause: WinRM is accessible over HTTP (port 5985) from any IP.

    Fix:

        Use WinRM over HTTPS (port 5986) with a valid certificate.

        Restrict WinRM access using Windows Firewall to only allow management subnets.

        Use CredSSP or Kerberos authentication where possible, not Basic Auth.

4. Disable NTLM Authentication (if possible)

    Root Cause: Windows sends NTLM hashes over the network when accessing UNC paths.

    Fix: In a pure Active Directory environment, you can disable NTLM and force Kerberos. It's more secure and doesn't leak hashes.

5. Network Segmentation

    Root Cause: The web server could reach out to an attacker's SMB share.

    Fix: Block outbound SMB (port 445) from web servers. They don't need it. Also, block SMB inbound from untrusted networks.

Final Thoughts

This machine is a perfect example of why low-hanging fruit is still a thing. A simple LFI + weak password + WinRM = complete compromise. It's a reminder that security is a chain, and the chain is only as strong as its weakest link.

And as Musashi said:

    "The way of the warrior is resolute acceptance of death."

Well, maybe not death, but at least accepting that badminton is not a password.

GG. See you on the next box.

Write-up by 0xAkram | HTB Player #141897